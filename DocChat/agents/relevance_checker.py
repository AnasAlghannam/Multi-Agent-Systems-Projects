...
class RelevanceChecker:
    def __init__(self):
        # Only needs to emit one short label, so keep it deterministic and small.
        self.model = get_llm(temperature=0.0, max_tokens=10)
    ...
    def check(self, question: str, retriever, k=3) -> str:
        ...
        **Respond ONLY with one of the following labels: CAN_ANSWER, PARTIAL, NO_MATCH**
        ...
        # Call the LLM
        try:
            llm_response = complete(self.model, prompt).upper()
