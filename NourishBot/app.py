...
from src.llm import vision_available
...
def analyze_food(image, dietary_restrictions, workflow_type, manual_ingredients="",
    ...
    :param manual_ingredients: Comma-separated ingredients, used when no vision
        model is configured
    :return: Result from the NourishBot workflow.
    """
    # The image step needs a vision model. Without one, fall back to the
    # ingredients the user typed - the rest of the pipeline is unchanged.
    if not vision_available():
        if not (manual_ingredients or "").strip():
            return (
                "### No vision model configured\n\n"
                "Set `NOURISH_VISION_MODEL` in `.env` to a vision-capable model, "
                "or type the ingredients into the **Ingredients** box and press "
                "Analyze again."
            )
        image_path = manual_ingredients.strip()
    else:
        if image is None:
            return "Upload an image, or type ingredients if you have no vision model."
...
with gr.Blocks(theme=gr.themes.Citrus(), css=css, js=js) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=400):
            ...
            manual_input = gr.Textbox(
                label="Ingredients (used when no vision model is configured)",
                placeholder="e.g. tomato, mozzarella, basil, olive oil",
                lines=2,
                visible=not vision_available(),
            )
    ...
    submit_btn.click(
        fn=analyze_food,
        inputs=[image_input, dietary_input, workflow_radio, manual_input],
...
if __name__ == "__main__":
    demo.launch(
        ...
        server_port=int(os.environ.get("APP_PORT", 7862)),
